...
- 링크 정규화와 `hashKey` 생성
- 신규 공지 중복 검사
- 해당 사이트에 저장된 공지가 0개인 초기 수집은 기준 데이터 저장만 수행하고 Discord 알림은 보내지 않음
...
- 기본 크롤링 주기는 30분이다.
- 공지 사이트 설정 저장 또는 교체 직후 해당 사이트만 즉시 수집한다.
- 해당 사이트의 첫 수집은 기준 데이터 저장용으로 처리하고 알림을 보내지 않는다.
